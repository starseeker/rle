...
#ifndef lint
static char rcsid[] = "$Id: giftorle.c,v 3.0 90/08/03 15:17:24 spencer Exp $";
#endif
...
#define	EasyFail(str,status)	while(1){fprintf(stderr,str);return(status);}
#define	HardFail(str,status)	while(1){fprintf(stderr,str);exit  (status);}
