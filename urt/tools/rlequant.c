...
static void mem_alloc(), read_input();
 ...
 * Usage:
 * 	rlequant [-b bits] [-c colors] [-d] [-f] [-o outfile] [infile]
 * Inputs:
 ...
 * 			allocated.  Must be <= 8.
 * 	-c colors:	The number of colors to quantize the image to.
...
char **argv;
{
    ...
    if ( scanargs( argc, argv,
		   "% b%-bits!d c%-colors!d d%- f%- o%-outfile!s infile%s",
		   &bflag, &bits, &cflag, &colors, &dflag, &fflag,
    ...
    colormap[0] = (rle_pixel *)malloc( colors );
    colormap[1] = (rle_pixel *)malloc( colors );
    colormap[2] = (rle_pixel *)malloc( colors );

    /* bytecount gives the size of the rgbmap, and is equal to (2^bits)^3
     * Allocate memory for the rgbmap.
     */
    bytecount = 1 << bits;
    bytecount *= bytecount * bytecount;
    rgbmap = (rle_pixel *)malloc( bytecount );
    if ( rgbmap == NULL )
    {
	fprintf( stderr, "%s: Unable to allocate RGB map memory.\n",
		 MY_NAME );
	exit( RLE_NO_SPACE );
    }
	...
	if ( in_hdr.ncolors < 3 )
	{
	    fprintf( stderr, "%s: Input image %d has only %d colors.\n",
	...
	mem_alloc( &in_hdr, entries, dflag, &rows, &red, &green, &blue, &alpha,
		   &img_red, &img_green, &img_blue );

	/* Read and quantize the input image. */
	read_input( &in_hdr, width, shift, dflag, red, green, blue, alpha,
		    img_red, img_green, img_blue, rows );

      /* Compute the color quantization map.  Dither flag implies that the
       * RGBmap must be built for multiple images.
       */
	colors = colorquant( red, green, blue, entries, colormap, colors, bits,
                           rgbmap, fflag, dflag );

	/* Compute and write setup information to the output file. */
	setup_output( &out_hdr, width, colors, colormap, outrows );
...
rle_pixel **img_red, **img_green, **img_blue;
{
    ...
    *red = (rle_pixel *)malloc( entries );
    *green = (rle_pixel *)malloc( entries );
    *blue = (rle_pixel *)malloc( entries );
    if ( hdr->alpha )
	*alpha = (rle_pixel *)malloc( entries );

    if ( (hdr->alpha && (*alpha == NULL)) || 
	...
	*img_red = (rle_pixel *)malloc( entries );
	*img_green = (rle_pixel *)malloc( entries );
	*img_blue = (rle_pixel *)malloc( entries );
...
static void
setup_output( hdr, width, colors, colormap, outrows )
rle_hdr *hdr;
int colors, width;
...
rle_pixel *outrows[2];
{
    ...
    hdr->cmap = (rle_map *)malloc( 3 * 256 * sizeof( rle_map ) );
    hdr->ncmap = 3;
    hdr->cmaplen = 8;
    for ( y = 0; y < 3; y++ )
    {
    ...
    outrows[1] = (rle_pixel *)malloc( width );
    if ( outrows[1] == NULL )
    {
	fprintf( stderr, "%s: Unable to allocate scanline memory.\n",
		 MY_NAME );
...
rle_pixel *outrows[2];
{
	...
	thisline = (short *) malloc( width * 3 * sizeof(short));
	nextline = (short *) malloc( width * 3 * sizeof(short));
	nextptr = nextline;
	for (x=0; x < width; x++)
	{
	    *nextptr++ = *irp++ ;
