...
double new_gamma;
{
	...
	if ( the_hdr->ncmap == 1 )	/* make "b&w" map */
	{
	    nmap = (minmap < the_hdr->ncolors) ?
		the_hdr->ncolors : minmap;
	    cmap = (rle_pixel **)malloc( nmap * sizeof(rle_pixel *) );
	    cmap[0] = (rle_pixel *)malloc( maplen * sizeof(rle_pixel) );
	    for ( i = 0; i < maplen; i++ )
		if ( i < cmaplen )
		    cmap[0][i] = the_hdr->cmap[i] >> 8;
		else
		    cmap[0][i] = i;
	    for ( i = 1; i < nmap; i++ )
		cmap[i] = cmap[0];
	...
	else if ( the_hdr->ncolors <= the_hdr->ncmap )
	{
	    nmap = (minmap < the_hdr->ncmap) ? the_hdr->ncmap : minmap;
	    cmap = (rle_pixel **)malloc( nmap * sizeof(rle_pixel *) );
